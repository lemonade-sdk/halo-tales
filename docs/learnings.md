...
| Setting Tauri 2's `app.windows[].decorations: false` (to roll a custom title bar) added orange underlines under every button in the native `<audio controls>` element (rewind / play / forward / volume / settings). The underlines live inside the audio element's shadow DOM and are not reachable by document-level CSS. | The change is rendering-only, not anything we wrote — WebKitGTK swaps in a different media-controls stylesheet under client-side decorations. Either keep decorations on (and accept the OS title bar) or build a custom audio control. Spending time grepping for `text-decoration` or `outline` in our CSS will find nothing; this is a webview-level side effect of disabling decorations. |
