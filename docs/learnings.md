...
| kokoro TTS returned `HTTP 200 OK` with a **zero-byte body** in 38 ms (vs the usual 5 s) on one continuation turn. Direct `curl` immediately afterwards worked fine. | Treat 200-empty as a one-shot backend hiccup: `textToSpeech` retries once inside the function, and `dispatchTool` catches persistent failure so the turn saves with image + prose + wiki (no audio) instead of stalling the agent loop. |
...
| Setting Tauri 2's `app.windows[].decorations: false` (to roll a custom title bar) added orange underlines under every button in the native `<audio controls>` element (rewind / play / forward / volume / settings). The underlines live inside the audio element's shadow DOM and are not reachable by document-level CSS. | The change is a webview-level side effect — WebKitGTK swaps in a different media-controls stylesheet under client-side decorations — so no amount of grepping our own CSS will turn up a fix. The way out is to render a custom audio player: a controls-less `<audio>` driven via React state from `onPlay` / `onPause` / `onTimeUpdate` / `onLoadedMetadata`, plus your own play/pause button and a `<input type="range">` scrub bar styled to taste. We did that in `src/components/AudioPlayer.tsx`. Bisecting tip: when something visual breaks but the CSS diff looks innocent, flip the most recent platform-level config flag (here, `decorations`) and see if the symptom disappears. |
...
| When iter 1 emits empty `msg.content` but calls `text_to_speech` with non-empty `input`, that input IS the prose the model wanted to display — it just routed it into the tool arg instead of into the assistant message. Without intervention we'd waste ~9 s on iter 2 and ~5 s on a re-TTS, both producing prose that desyncs from the audio. | After dispatching tools, if `output.narration` is empty but `ctx.audioSourceText` has content, harvest the TTS input as the displayed prose. Same string for display and audio → in-sync by construction, single iteration, ~15 s saved per turn. |
| Audio desync repair re-narrated audio that differed from the displayed prose by **one character** (trailing whitespace), wasting ~5 s. | Normalize both strings to single-space-collapsed-and-trimmed before comparing. Real divergence still triggers a re-TTS; trivial whitespace doesn't. |
| Timeline card showed "Turn 3" when only two cards were visible — `entry.seq` counts user inputs as their own seq, so the storyteller's second card is seq 3. | Group adjacent `user` + `scene` entries into "rounds" at render time; display the 1-indexed round number, not `entry.seq`. The wiki Timeline tab still lists raw seqs because it's the editable per-entry view. |
